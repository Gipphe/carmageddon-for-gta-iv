		...
		Sub gtaivcarrier()
			quitting = false
			confirmpath handlingpath,false
		...
		Sub eflccarrier()
			quitting = false
			confirmpath EFLChandlingpath,true
			confirmpath TBoGThandlingpath,true
			confirmpath TLADhandlingpath,true
			if quitting = false then
				installfiles(true)
				if quitting = false then
					scriptrun(true)
		...
		Sub findpath(iseflc)
			If iseflc = true then
				If objFSO.FileExists(EFLCconfig) Then
					Set objFile = objFSO.OpenTextFile(EFLCconfig, ForReading)
